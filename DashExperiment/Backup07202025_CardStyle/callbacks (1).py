...
def register_callbacks(app, osa_thread_ref, fig, current_figurl_ref):
    ...
    def control_scan_mode(repeat_clicks, single_clicks, stop_clicks):
        ...
        default_style = {'marginRight': 10, 'backgroundColor': '#f0f0f0', 'border': '1px solid #ccc', 'padding': '8px 16px'}
        active_style = {'marginRight': 10, 'backgroundColor': '#4CAF50', 'color': 'white', 'border': '1px solid #4CAF50', 'padding': '8px 16px'}
        
        # Check if OSA is connected
        if not (osa_thread and osa_thread.is_alive() and osa_thread.osa.connected):
            return default_style, default_style, default_style, "Scan Mode: OSA Not Connected"
        ...
        if not ctx.triggered:
            ...
            if hasattr(osa_thread, 'current_scan_mode'):
                ...
                if current_mode == "repeat":
                    return active_style, default_style, default_style, "Scan Mode: Repeat"
                elif current_mode == "single":
                    return default_style, active_style, default_style, "Scan Mode: Single"
                elif current_mode == "stop":
                    return default_style, default_style, active_style, "Scan Mode: Stopped"
            return default_style, default_style, default_style, "Scan Mode: Unknown"
        ...
        try:
            ...
            return default_style, default_style, default_style, f"Error: {str(e)}"
