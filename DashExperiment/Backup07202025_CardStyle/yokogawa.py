...
class Yokogawa(object):
    ...
    def trace(self):
        try:
            ...
            xread =  self._QuerryData(MESSAGE, self._TRACE_SIZE)
            x = [float(xx) for xx in xread.decode().split(',')]

            MESSAGE = ":TRACe:DATA:Y? TRA\r\n"
            yread = self._QuerryData(MESSAGE, self._TRACE_SIZE)
    ...
    def settings(self):
        if not self._connected:
            ...
            pts_auto_raw = float(self._QuerryData(":SENSe:SWEep:POINts:AUTO?\n", 256).strip())
            self._params['pts_auto'] = bool(pts_auto_raw)  # Convert to proper boolean
            self._params['resolution'] = float(self._QuerryData(":SENSe:BANDwidth?\n", 256).strip())
            calib_zero_raw = float(self._QuerryData(":CALibration:ZERO:AUTO?\n", 256).strip())
            self._params['calib_zero'] = bool(calib_zero_raw)  # Convert to proper boolean
    ...
    def settings(self, val):
        ...
        try:
            ...
            # Handle points setting
            if self._params['pts_auto']:
                # Set automatic points mode
                success &= self._Write_OSA(":SENSe:SWEep:POINts:AUTO ON\n")
            else:
                # Set manual points mode
                success &= self._Write_OSA(":SENSe:SWEep:POINts:AUTO OFF\n")
                if self._params['pts'] is not None:
