...
PmeterList = dict(IN = None, OUT =None)
connected = dict(IN= False, OUT= False)

async def getP(instr):
    try:
        P = await instr._get_power_async()
    except:
        P = 0
    return P


async def _getStatus(ui_slot, in_out):
    if ui_slot.Cnct.value:
        if not connected[in_out]:
            instr = connected_instr[ui_slot.Det.value]
            PmeterList[in_out] = ThorlabsP1xx(address = instr)   
            PmeterList[in_out].connected = True
            PmeterList[in_out].lbd = float(ui_slot.λ.value)
            connected[in_out] = True
            print(f'Connected to {instr}')
        else:
            PmeterList[in_out].connected = False
            PmeterList[in_out] = None
    else:
        try: 
            PmeterList[in_out].connected = False
        except: 
...
def displayPower(*Ps):
    for pp, ui_slot in zip(Ps, [ui.IN, ui.OUT]):
        val = pp*1e3 * ui_slot.fact.value
        if val>10:
    ...
    if not(np.nan in Ps):
            outP = float(ui.OUT.P.value.replace(' mW', ''))
            inP = float(ui.IN.P.value.replace(' mW', ''))
            if outP>0:
                ui.Losses.loss.value = '{:.3f} dB'.format(10*np.log10(outP/inP))
            else:
                ui.Losses.loss.value = '-Inf dB'
    
async def getIL():
    # -- loop until runner closed -- 
    total = 100
    while True: 
        ui.text.append_stdout(f'{ui.running.value}-')
        # -- Check status --
        await _getStatus(ui.IN, 'IN')
        await _getStatus(ui.OUT, 'OUT')
        
        # -- check if connected or not: 
        
        Pmeters = (getP(xx) for xx in PmeterList.values())
        Pin, Pout = await asyncio.gather(*Pmeters)        
        displayPower(Pin, Pout)
